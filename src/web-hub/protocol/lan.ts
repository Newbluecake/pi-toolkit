...
// ---------------------------------------------------------------------------
// L8 密码长度（hub 侧强制 admin.ts + TUI 客户端预检 agent/passwd-prompt.ts 共享同一常量，
// 避免两处口径不一致：客户端拒绝不代表服端真正拒绝，反之服端拒绝而客户端未拦下会把用户已输入密码提前发往 hub）
// ---------------------------------------------------------------------------

export const MIN_LAN_PASSWORD_LENGTH = 10;
export const MAX_LAN_PASSWORD_LENGTH = 256;
