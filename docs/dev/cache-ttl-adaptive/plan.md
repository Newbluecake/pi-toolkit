...
### 19.3 第三轮评审修订（REJECT：1 Major / 4 Minor；R7、R8、R2 保守策略已确认成立）

| 意见                                                                                    | 裁决     | 修订                                                                                                                                                    |
| --------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Major：账本缺 `provider` 时路由键退化为 `\|model`，同模型名的不同 provider 共用生存证据 | 采纳     | `ledgerRouteKey` 缺 provider 或 model ⇒ `undefined`；未知路由上的命中照常计 `indirect1hConfirms`，但**清空**生存证据（可能已换 provider），永不触发让位 |
| 32-bit FNV 生日碰撞                                                                     | 采纳     | 改 53-bit cyrb53（零依赖）                                                                                                                              |
| 工具 JSON 属性顺序敏感                                                                  | 不采纳   | 上游缓存按**字节前缀**匹配：同语义不同字节本来就是不同缓存谱系，规范化反而会掩盖真实分裂；pi 自身序列化稳定                                             |
| 循环对象回退为工具数量                                                                  | 采纳     | 不可序列化 ⇒ `payloadLineageKey` 返回 `undefined`（谱系未知、不检查），不用有损替身                                                                     |
| R7 接口语义依赖调用方                                                                   | 部分采纳 | `gapHorizonMs` 注释写明契约：描述下一请求开启的窗口的能力，当前请求可否流式由持有载荷的调用方判断；#7.5 本身已有单测，不另加端到端 ping 测试            |

## 20. 策略矩阵验证：武装空档、1h 寿命学习、桥接空档不判决（2026-09-26）

多场景策略矩阵（5m / 5m+ka / 1h / adaptive / adaptive+ka × 12 负载 × 3 档 1h 寿命 × 2 档读价）暴露三处缺陷，
先在仿真里原型化验证、再改代码。详见 `verification-2026-09-26.md`，要点（本节为准）：

- **F-D**：F1 `keepalive-covers` 只看 `armedGaps`（保活证明过读中的空档）；未被 ping 的人为长空档不再让 F1 整会话失效。
- **F-A**：已覆盖续期读崩塌 ⇒ 学到路由 1h 寿命上界 `learned1hLifeMs`，cover = `min(1h, ADAPTIVE_COVER_SAFETY(0.8) × 上界)`，跨 /reload 恢复（路由绑定）。
- **F-C**：`lastGapPingBridged` 的空档不做 1h 判决、不记生存证据；空档环仍用真实请求间隔（只改判决，否则 F1 失去 >49m 证据）。
- F-B（首请求绕过 keepalive-covers）是仿真器未建模 gate #8 造成的假象，撤销；R1 维持原样。
