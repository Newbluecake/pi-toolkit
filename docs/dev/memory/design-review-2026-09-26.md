...
## 6A. 工具层建议（2026-09-26 补充，主会话与用户讨论后追加）

§2–§6 主要处理「注入什么」。工具层决定「找得多快、改得多准、写入时能否卡住体积」，与注入层互补。

### 6A.1 现状问题

当前 `memory` 工具（`src/memory/tool.ts`）只有 `list` / `write`（整文件覆盖）/ `append` 三个 action，读取依赖通用 `read` + 绝对路径。

1. **只增不改**：改一条过时规则要么 `write` 重写整个文件（改 5KB 文件付 5KB 输出 token），要么继续 `append`，新旧说法并存。文件只会膨胀——这正是「需要自动整理」的根因。
2. **查找多次往返**：`list` → 猜文件 → `read` 全文，为一条事实付 2–3 轮往返加整文件 token。
3. **写入无反馈**：写完不知道文件多大、是否超预算、是否与已有条目重复，只能靠事后体检/整理补救。
4. **派发子 agent 无法指定记忆**：子会话要么全量注入、要么完全没有；真正知道子任务需要哪个主题的是派发它的主会话。

### 6A.2 建议（按收益/成本排序）

| #   | 做法                                                                                                               | 解决的问题                                             | 成本                                  |
| --- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------ | ------------------------------------- |
| T1  | 增加 `str_replace` / `insert` / `delete` / `rename`；`view` 支持行号范围或按 `##` 标题读取一段                     | 改一条只付这一条的 token；过时条目可直接删；从源头止涨 | 低                                    |
| T2  | 增加 `search` action：关键词检索 memory 目录，返回「文件 › 标题 › 命中行」坐标                                     | 一次调用定位到具体条目，不再 list→猜→读全文            | 低                                    |
| T3  | 写入结果携带预算与查重信息（如 `core 1.4/1.6KB；与 quota.md:3 近似重复`），超硬上限拒写并给出拆分建议              | 模型写入当下自我约束，后置整理需求大幅减少             | 低                                    |
| T4  | `Agent({ memory: ["cache-ttl"] })`：子会话默认 core-only，由派发方点名追加主题（与 §6 P0-4 的 child profile 配合） | 子会话 memory 开销从全量降到按需                       | 中                                    |
| T5  | （可选）按 `read_when` 关键词预取：用户消息命中某主题关键词时，把该主题相关段落作为尾部消息附上                    | 相关时零工具调用命中；不相关时零 token；尾部不破坏缓存 | 中；有误触发风险，需开关 + 命中率统计 |

**命令集对齐官方 memory 工具。** Anthropic 官方 memory 工具（`memory_20250818`，文档 <https://platform.claude.com/docs/en/agents-and-tools/tool-use/memory-tool>）的命令为 `view` / `create` / `str_replace` / `insert` / `delete` / `rename`，`str_replace` 要求 `old_str` 唯一、省略 `new_str` 即删除，成功返回带行号的编辑片段。T1 建议直接沿用这套命名与返回约定——模型对它更熟悉，调用出错更少；现有 `list` / `write` / `append` 保留为兼容别名。注意 pi 下这是自定义工具（我们自己的 schema），不是 Anthropic 服务端内置工具类型；只借用命名与语义，不依赖 provider。

**不建议**：写入时同步调用 LLM 总结/整理——增加每次写入的延迟、成本与失败面。整理保持显式触发或后台流程（§4.3、§6 P2-8）。

### 6A.3 与注入层的配合

注入层负责「精简 core + 带描述的索引」，让模型一眼知道有什么、何时该读；工具层负责「找得快、改得准、写入即卡体积」。合起来的目标：

- 常见情况零工具调用（core 覆盖）；
- 主题查询一次调用（`search` 或按段 `view`）；
- 记忆不膨胀（写入预算 + `str_replace` / `delete` 就地修改）；
- 子会话只带需要的主题（T4）。

对 todo #22 的影响：T1–T3 应并入 §6 的 P1（写入约束），且优先于「自动整理」——先让记忆不再只增不减，自动整理的必要性与规模都会下降；T4 并入 §6 P0-4；T5 放到 P2，先有 §6 P2-9 的指标再决定是否默认开启。
